...
/**
 * Hero Section - Video zoom and content fade effects
 */
class HeroScrollHandler implements ScrollHandler {
	private heroSection: HTMLElement | null = null;
	private videoContainer: HTMLElement | null = null;
	private heroContent: HTMLElement | null = null;
	private floatingStats: HTMLElement | null = null;
	...
	init(): void {
		...
		this.floatingStats = document.getElementById("floating-stats");
	...
	private handleScroll(): void {
		...
		this.videoContainer.style.opacity = videoOpacity.toString();

		// Content slide up effect
		const contentTranslateY = progress * -100;
		const contentOpacity = 1 - progress * 1.5;
		this.heroContent.style.transform = `translateY(${contentTranslateY}px)`;
		this.heroContent.style.opacity = contentOpacity.toString();

		// Stats fade out effect
		const statsOpacity = 1 - progress * 2;
		this.floatingStats.style.opacity = statsOpacity.toString();
	}
